...
print("RUNNING Movie Reviews SCRIPT v16 — QWEN API TAMIL PARALLEL (KEYWORDS + DIVERSITY)")

INPUT_JSON_PATH = "/Users/user/Desktop/IndicDetect/Benchmark_Data/Human_Written/Tamil/Movie_Reviews/tamil_movie_reviews_1000_400-500_20260312_211631.jsonl"
OUTPUT_JSON_PATH = INPUT_JSON_PATH.rsplit(".", 1)[0] + "_movie_reviews_articles_Qwen.json"
...
def build_keyword_prompt(sample_text):
    ...
    return (
        f"கீழே உள்ள திரைப்பட விமர்சனப் பகுதியிலிருந்து துல்லியமாக 4 முக்கியச் சொற்களை தமிழில் வழங்கவும். "
        f"இந்த முக்கியச் சொற்கள் கதை அம்சங்கள், நடிப்பு, தொழில்நுட்ப அம்சங்கள் (இயக்கம், இசை), "
        f"மற்றும் உணர்ச்சித் தாக்கம் ஆகியவற்றுடன் தொடர்புடையவையாக இருக்க வேண்டும். "
        f"விளக்கங்கள் அல்லது பிற வாக்கியங்கள் எழுத வேண்டாம்:\n{sample_text}"
    )
...
def build_article_prompt(keywords):
    ...
    return (
        f"இந்த 4 முக்கியச் சொற்களை அடிப்படையாகக் கொண்டு முழுவதும் தமிழ் மொழியில் ஒரு முழுமையான திரைப்பட விமர்சனத்தை எழுதவும். "
        f"விமர்சனம் அதிகாரபூர்வ விமர்சகர் பாணியில் இருக்க வேண்டும். "
        f"குறைந்தது 30 வாக்கியங்கள் இருக்க வேண்டும். "
        f"விமர்சனத்தில் கீழ்க்கண்ட அம்சங்கள் தெளிவாக இருக்க வேண்டும்:\n"
        f"- கதையமைப்பு மற்றும் திரைக்கதை பகுப்பாய்வு\n"
        f"- நடிகர்/நடிகையரின் நடிப்பு மதிப்பீடு\n"
        f"- இயக்கம், ஒளிப்பதிவு மற்றும் இசை விமர்சனம்\n"
        f"- திரைப்படத்தின் மொத்த உணர்ச்சித் தாக்கம்\n"
        f"அதே சொற்களை மீண்டும் மீண்டும் பயன்படுத்த வேண்டாம். பல்வகை வாக்கிய அமைப்பைப் பயன்படுத்தவும். "
        f"மேற்கோள்கள், விளக்கங்கள், அல்லது முன்னுரை வழங்க வேண்டாம். விமர்சனத்தை மட்டும் எழுதவும்.\n\n"
        f"முக்கியச் சொற்கள்:\n{keywords}\n\nதிரைப்பட விமர்சனம்:"
    )
...
def build_extend_prompt(keywords, existing_text):
    ...
    return (
        f"இந்த முக்கியச் சொற்களை அடிப்படையாகக் கொண்டு கீழே உள்ள தமிழ் திரைப்பட விமர்சனத்தை தொடரவும். "
        f"ஏற்கனவே விவாதித்த அம்சங்களை மீண்டும் எழுத வேண்டாம். "
        f"புதிதாக கீழ்க்கண்ட அம்சங்களை பகுப்பாய்வு செய்யவும்:\n"
        f"- இன்னும் விவாதிக்கப்படாத நடிப்பு அல்லது தொழில்நுட்ப அம்சங்கள்\n"
        f"- காட்சிகளின் ஆழமான விமர்சனப் பகுப்பாய்வு\n"
        f"- திரைப்படத்தின் பலங்கள் மற்றும் பலவீனங்கள்\n"
        f"- மொத்த விமர்சனத் தீர்ப்பு மற்றும் மதிப்பீடு (ரேட்டிங்) பரிந்துரை\n"
        f"அதிகாரபூர்வ விமர்சகர் பாணி தொடர வேண்டும். "
...
def generate_article(client, tokenizer, sample_text):
    ...
    Pipeline:
      ...
      2. Use keywords → generate full academic article
...
def process_single_sample(client, tokenizer, idx, sample_text):
    ...
    # Fallback 3: Completely generic Tamil academic article
    print(f"  Sample #{idx}: trying generic fallback...")
    generic_prompt = (
    "முழுவதும் தமிழ் மொழியில் ஒரு முழுமையான திரைப்பட விமர்சனத்தை மட்டும் எழுதவும். "
    "குறைந்தது 30 வாக்கியங்கள் இருக்க வேண்டும். திரைப்பட விமர்சனத்தை மட்டும் எழுதவும்."
)
    for _ in range(3):
...
def main():
    ...
    if not api_key:
        raise SystemExit("Missing DASHSCOPE_API_KEY environment variable")
