...
print("RUNNING Movie Reviews SCRIPT v16 — QWEN API HINDI PARALLEL")

INPUT_JSON_PATH = "/Users/user/Desktop/IndicDetect/Benchmark_Data/Human_Written/Hindi/Movie_Reviews/hindi_reviews_1000.jsonl"
...
def clean_text(text):
    ...
    for part in re.split(r'(?<=[.।!?॥])\s*', text):
        ...
        if not part:
            continue
        hindi_chars = sum(1 for ch in part if '\u0900' <= ch <= '\u097F')
        total_chars = len(part.replace(" ", ""))
        if total_chars > 0 and hindi_chars / total_chars >= 0.4:
...
def build_keyword_prompt(sample_text):
    ...
    return (
        f"निम्नलिखित फिल्म समीक्षा अंश से ठीक 4 मुख्य शब्द हिंदी में दीजिए। "
        f"ये मुख्य शब्द कथानक, अभिनय, तकनीकी पहलुओं (निर्देशन, संगीत), "
        f"और भावनात्मक प्रभाव से संबंधित होने चाहिए। "
        f"कोई विवरण या अन्य वाक्य न दें:\n{sample_text}"
    )


def build_article_prompt(keywords):
    """Step 2: Generate a full formal Hindi movie review from those 4 keywords."""
    return (
        f"इन 4 मुख्य शब्दों के आधार पर पूर्णतः हिंदी भाषा में एक संपूर्ण फिल्म समीक्षा लिखें। "
        f"समीक्षा आधिकारिक आलोचनात्मक शैली में होनी चाहिए। "
        f"कम से कम 30 वाक्य होने चाहिए। "
        f"समीक्षा में निम्नलिखित पहलू स्पष्ट रूप से होने चाहिए:\n"
        f"- कथानक और पटकथा विश्लेषण\n"
        f"- कलाकारों के अभिनय का मूल्यांकन\n"
        f"- निर्देशन, सिनेमाटोग्राफी और संगीत की समीक्षा\n"
        f"- फिल्म का समग्र भावनात्मक प्रभाव\n"
        f"एक ही शब्दों को बार-बार उपयोग न करें। विविध वाक्य संरचना का उपयोग करें। "
        f"निर्देश, विवरण, या भूमिका न दें। केवल समीक्षा ही लिखें।\n\n"
        f"मुख्य शब्द:\n{keywords}\n\nफिल्म समीक्षा:"
    )
...
def build_extend_prompt(keywords, existing_text):
    ...
    return (
        f"इन मुख्य शब्दों के आधार पर निम्नलिखित हिंदी फिल्म समीक्षा को आगे बढ़ाएँ। "
        f"पहले से चर्चित अंशों को दोबारा न लिखें। "
        f"नए रूप में निम्नलिखित पहलुओं का विश्लेषण करें:\n"
        f"- अभी तक चर्चा न किए गए अभिनय या तकनीकी पहलू\n"
        f"- दृश्यों का गहन आलोचनात्मक विश्लेषण\n"
        f"- फिल्म की शक्तियाँ और कमज़ोरियाँ\n"
        f"- समग्र समीक्षा निर्णय और रेटिंग सुझाव\n"
        f"आधिकारिक आलोचनात्मक शैली बनी रहनी चाहिए। "
        f"कम से कम 15 नए वाक्य लिखें।\n\n"
        f"मुख्य शब्द: {keywords}\n\n"
        f"अब तक:\n{existing_text}\n\nसमीक्षा आगे लिखें:"
    )
...
def process_single_sample(client, tokenizer, idx, sample_text):
    ...
    generic_prompt = (
    "पूर्णतः हिंदी भाषा में एक संपूर्ण फिल्म समीक्षा ही लिखें। "
    "कम से कम 30 वाक्य होने चाहिए। केवल फिल्म समीक्षा ही लिखें।"
    )    
    for _ in range(3):
    ...
    return idx, "शैक्षणिक लेख उपलब्ध नहीं है।", 5
