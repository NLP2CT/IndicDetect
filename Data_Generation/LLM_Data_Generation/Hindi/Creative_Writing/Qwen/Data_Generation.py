...
print("RUNNING Creative Writing SCRIPT v16 — QWEN API HINDI PARALLEL")

INPUT_JSON_PATH = "/Users/user/Desktop/IndicDetect/Benchmark_Data/Human_Written/Hindi/Creative_Writing/hindi_refined.jsonl"
OUTPUT_JSON_PATH = INPUT_JSON_PATH.rsplit(".", 1)[0] + "_creative_articles_Qwen.json"
...
def build_keyword_prompt(sample_text):
    """Step 1: Extract 4 literary/thematic keywords from the input sample to understand its narrative."""
    return (
        f"निम्नलिखित पाठ से ठीक 4 साहित्यिक या कथापरक मुख्य शब्द "
        f"हिंदी में दीजिए। कोई विवरण या अन्य वाक्य न दें:\n{sample_text}"
    )


def build_article_prompt(keywords):
    """Step 2: Generate a full novel-style narrative passage from those 4 keywords."""
    return (
        f"इन 4 मुख्य शब्दों के आधार पर पूर्णतः हिंदी भाषा में "
        f"एक नया, विस्तृत उपन्यास शैली का गद्य अंश ही लिखें। "
        f"कम से कम 30 वाक्य होने चाहिए। "
        f"पात्रों की भावनाएँ, वर्णनात्मक दृश्य, स्वाभाविक संवाद, "
        f"और कथा की गति स्पष्ट होनी चाहिए। "
        f"एक ही शब्दों को बार-बार उपयोग न करें। विविध वाक्य संरचना का उपयोग करें। "
        f"निर्देश, विवरण, या भूमिका न दें। केवल उपन्यास शैली का गद्य अंश ही लिखें।\n\n"
        f"मुख्य शब्द:\n{keywords}\n\nनया उपन्यास शैली का गद्य अंश:"
    )


def build_extend_prompt(keywords, existing_text):
    """Step 3 (if needed): Extend an existing novel passage with NEW narrative content."""
    return (
        f"इन मुख्य शब्दों के आधार पर निम्नलिखित हिंदी उपन्यास शैली के गद्य अंश को आगे बढ़ाएँ। "
        f"पहले से लिखे गए दृश्यों को दोबारा न लिखें। "
        f"केवल नए दृश्य, पात्रों की मानसिक स्थिति, नई घटनाएँ और "
        f"कथा में मोड़ ही जोड़ें। "
        f"उपन्यास शैली बनी रहनी चाहिए। "
...
def process_single_sample(client, tokenizer, idx, sample_text):
    ...
    generic_prompt = (
    "पूर्णतः हिंदी भाषा में एक नया, विस्तृत उपन्यास शैली का गद्य अंश ही लिखें। "
    "कम से कम 30 वाक्य होने चाहिए। केवल उपन्यास शैली का गद्य अंश ही लिखें।"
   )
    for _ in range(3):
