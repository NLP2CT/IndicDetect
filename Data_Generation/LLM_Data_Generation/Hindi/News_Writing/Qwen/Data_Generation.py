...
print("RUNNING News Writing SCRIPT v16 — QWEN API HINDI PARALLEL")

INPUT_JSON_PATH = "/Users/user/Desktop/IndicDetect/Benchmark_Data/Human_Written/Hindi/News_Writing/hindi_1000_400-500.jsonl"
OUTPUT_JSON_PATH = INPUT_JSON_PATH.rsplit(".", 1)[0] + "_news_articles_Qwen.json"
...
def build_keyword_prompt(sample_text):
    """Step 1: Extract 4 keywords from the input sample to understand its topic."""
    return (
        f"निम्नलिखित समाचार कथन से ठीक 4 संक्षिप्त समाचार मुख्य शब्द "
        f"हिंदी में दीजिए। कोई विवरण या अन्य वाक्य न दें:\n{sample_text}"
    )


def build_article_prompt(keywords):
    """Step 2: Generate a full news article from those 4 keywords."""
    return (
        f"इन 4 मुख्य शब्दों के आधार पर पूर्णतः हिंदी भाषा में "
        f"एक नया, विस्तृत समाचार कथन ही लिखें। कथन में कम से कम 30 वाक्य होने चाहिए। "
        f"एक ही शब्दों को बार-बार उपयोग न करें। विविध वाक्य संरचना का उपयोग करें। "
        f"निर्देश, विवरण, या भूमिका न दें। केवल समाचार कथन ही लिखें।\n\n"
        f"मुख्य शब्द:\n{keywords}\n\nनया समाचार कथन:"
    )


def build_extend_prompt(keywords, existing_text):
    """Step 3 (if needed): Extend an existing article with NEW content using same keywords."""
    return (
        f"इन मुख्य शब्दों के आधार पर निम्नलिखित हिंदी समाचार कथन को आगे बढ़ाएँ। "
        f"पहले से लिखे गए अंशों को दोबारा न लिखें। "
        f"केवल नए विवरण, नए दृष्टिकोण, नई जानकारी जोड़ें। "
...
def process_single_sample(client, tokenizer, idx, sample_text):
    ...
    generic_prompt = (
    "पूर्णतः हिंदी भाषा में एक सामान्य समाचार कथन लिखें। "
    "कम से कम 30 वाक्य होने चाहिए। केवल समाचार कथन ही लिखें।"
    )
    for _ in range(3):
