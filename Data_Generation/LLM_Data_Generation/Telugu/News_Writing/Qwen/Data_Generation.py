...
print("RUNNING News Writing SCRIPT v16 — QWEN API TELUGU PARALLEL (KEYWORDS + DIVERSITY)")

INPUT_JSON_PATH = "/Users/user/Desktop/IndicDetect/Benchmark_Data/Human_Written/Telugu/News_Writing/telugu_1000_400-500.jsonl"
OUTPUT_JSON_PATH = INPUT_JSON_PATH.rsplit(".", 1)[0] + "_news_articles.json"
...
def build_keyword_prompt(sample_text):
    ...
    return (
        f"క్రింది వార్తా కథనం నుండి ఖచ్చితంగా 4 సంక్షిప్త వార్తా కీలక పదాలను "
...
def build_article_prompt(keywords):
    ...
    return (
        ...
        f"ఒక కొత్త, విశదమైన వార్తా కథనాన్ని మాత్రమే రాయండి. కథనం కనీసం 30 వాక్యాలు ఉండాలి. "
        f"అదే పదాలను మళ్ళీ మళ్ళీ ఉపయోగించకండి. వైవిధ్యమైన వాక్య నిర్మాణం ఉపయోగించండి. "
        f"సూచనలు, వివరణలు, లేదా ఉపోద్ఘాతం ఇవ్వకండి. వార్తా కథనం మాత్రమే రాయండి.\n\n"
        f"కీలక పదాలు:\n{keywords}\n\nకొత్త వార్తా కథనం:"
    )
...
def build_extend_prompt(keywords, existing_text):
    ...
    return (
        f"ఈ కీలక పదాల ఆధారంగా క్రింది తెలుగు వార్తా కథనాన్ని కొనసాగించండి. "
        f"ఇప్పటికే రాసిన అంశాలను మళ్ళీ రాయకండి. "
        f"కొత్త వివరాలు, కొత్త కోణాలు, కొత్త సమాచారం మాత్రమే జోడించండి. "
...
def process_single_sample(client, tokenizer, idx, sample_text):
    ...
    generic_prompt = (
        "పూర్తిగా తెలుగు భాషలో ఒక సాధారణ వార్తా కథనాన్ని రాయండి. "
        "కనీసం 30 వాక్యాలు ఉండాలి. వార్తా కథనం మాత్రమే రాయండి."
    )
    for _ in range(3):
