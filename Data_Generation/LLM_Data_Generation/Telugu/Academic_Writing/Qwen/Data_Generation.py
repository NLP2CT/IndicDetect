...
print("RUNNING Academic Writing SCRIPT v16 — QWEN API TELUGU PARALLEL (KEYWORDS + DIVERSITY)")

INPUT_JSON_PATH = "/Users/user/Desktop/IndicDetect/Benchmark_Data/Human_Written/Telugu/Academic_Writing/telugu_samples.jsonl"
OUTPUT_JSON_PATH = INPUT_JSON_PATH.rsplit(".", 1)[0] + "_academic_articles.json"
...
def build_keyword_prompt(sample_text):
    ...
    return (
        f"క్రింది పాఠ్య భాగం నుండి ఖచ్చితంగా 4 శాస్త్రీయ లేదా అకాడెమిక్ కీలక పదాలను "
...
def build_article_prompt(keywords):
    ...
    return (
        ...
        f"ఒక కొత్త, విశదమైన అకాడెమిక్ వ్యాసాన్ని మాత్రమే రాయండి. "
        f"వ్యాసం కనీసం 30 వాక్యాలు ఉండాలి. "
        f"పరిశోధనా శైలి, తర్కబద్ధమైన నిర్మాణం, స్పష్టమైన విశ్లేషణ ఉండాలి. "
        f"అదే పదాలను మళ్ళీ మళ్ళీ ఉపయోగించకండి. వైవిధ్యమైన వాక్య నిర్మాణం ఉపయోగించండి. "
        f"సూచనలు, వివరణలు, లేదా ఉపోద్ఘాతం ఇవ్వకండి. అకాడెమిక్ వ్యాసం మాత్రమే రాయండి.\n\n"
        f"కీలక పదాలు:\n{keywords}\n\nకొత్త అకాడెమిక్ వ్యాసం:"
    )
...
def build_extend_prompt(keywords, existing_text):
    ...
    return (
        f"ఈ కీలక పదాల ఆధారంగా క్రింది తెలుగు అకాడెమిక్ వ్యాసాన్ని కొనసాగించండి. "
        f"ఇప్పటికే రాసిన అంశాలను మళ్ళీ రాయకండి. "
        f"కొత్త విశ్లేషణ, కొత్త పరిశోధనా కోణాలు, కొత్త సమాచారం మాత్రమే జోడించండి. "
        f"తర్కబద్ధమైన శైలి కొనసాగాలి. "
...
def process_single_sample(client, tokenizer, idx, sample_text):
    ...
    generic_prompt = (
    "పూర్తిగా తెలుగు భాషలో ఒక కొత్త, విశదమైన అకాడెమిక్ వ్యాసాన్ని మాత్రమే రాయండి. "
    "కనీసం 30 వాక్యాలు ఉండాలి. అకాడెమిక్ వ్యాసం మాత్రమే రాయండి."
    )
    for _ in range(3):
