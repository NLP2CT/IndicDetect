...
print("RUNNING Creative Writing SCRIPT v16 — QWEN API TELUGU PARALLEL (KEYWORDS + DIVERSITY)")

INPUT_JSON_PATH = "/Users/user/Desktop/IndicDetect/Benchmark_Data/Human_Written/Telugu/Creative_Writing/telugu_refined.jsonl"
OUTPUT_JSON_PATH = INPUT_JSON_PATH.rsplit(".", 1)[0] + "_creative_articles.json"
...
def build_keyword_prompt(sample_text):
    ...
    return (
        f"క్రింది పాఠ్య భాగం నుండి ఖచ్చితంగా 4 సాహిత్యపరమైన లేదా కథాపరమైన కీలక పదాలను "
        f"తెలుగు లో ఇవ్వండి. వివరణలు లేదా ఇతర వాక్యాలు ఇవ్వకండి:\n{sample_text}"
    )
...
def build_article_prompt(keywords):
    ...
    return (
        f"ఈ 4 కీలక పదాల ఆధారంగా పూర్తిగా తెలుగు భాషలో "
        f"ఒక కొత్త, విశదమైన నవలా శైలి గద్య భాగాన్ని మాత్రమే రాయండి. "
        f"కనీసం 30 వాక్యాలు ఉండాలి. "
        f"పాత్రల భావోద్వేగాలు, వర్ణనాత్మక దృశ్యాలు, సహజమైన సంభాషణలు, "
        f"మరియు కథా గమనం స్పష్టంగా ఉండాలి. "
        f"అదే పదాలను మళ్ళీ మళ్ళీ ఉపయోగించకండి. వైవిధ్యమైన వాక్య నిర్మాణం ఉపయోగించండి. "
        f"సూచనలు, వివరణలు, లేదా ఉపోద్ఘాతం ఇవ్వకండి. నవలా గద్య భాగం మాత్రమే రాయండి.\n\n"
        f"కీలక పదాలు:\n{keywords}\n\nకొత్త నవలా గద్య భాగం:"
    )
...
def build_extend_prompt(keywords, existing_text):
    ...
    return (
        f"ఈ కీలక పదాల ఆధారంగా క్రింది తెలుగు నవలా గద్య భాగాన్ని కొనసాగించండి. "
        f"ఇప్పటికే రాసిన సన్నివేశాలను మళ్ళీ రాయకండి. "
        f"కొత్త సన్నివేశాలు, పాత్రల మానసిక స్థితి, కొత్త సంఘటనలు మరియు "
        f"కథలో మలుపులు మాత్రమే జోడించండి. "
        f"నవలా శైలి కొనసాగాలి. "
...
def process_single_sample(client, tokenizer, idx, sample_text):
    ...
    generic_prompt = (
        "పూర్తిగా తెలుగు భాషలో ఒక కొత్త, విశదమైన నవలా శైలి గద్య భాగాన్ని మాత్రమే రాయండి. "
        "కనీసం 30 వాక్యాలు ఉండాలి. నవలా గద్య భాగం మాత్రమే రాయండి."
    )
    for _ in range(3):
