...
def _auto_outfile() -> str:
    ...
    return f"tamil_{NUM_SAMPLES}_{MIN_TOK}-{MAX_TOK}_{ts}.jsonl"
...
TAMIL_NEWS_SITES = [
    {
        ...
        "archive_pages": [
            "https://www.dinamalar.com/news/tamilnadu",
            "https://www.dinamalar.com/news/india",
            "https://www.dinamalar.com/news/world",
            "https://www.dinamalar.com/news/sports",
            "https://www.dinamalar.com/news/health",
        ],
    },
    {
        "name": "Maalaimalar",
        ...
        "archive_pages": [
            "https://www.maalaimalar.com/tamilnadu",
            "https://www.maalaimalar.com/india",
            "https://www.maalaimalar.com/world",
        ],
    },
    {
        "name": "The Hindu Tamil",
        "sitemaps": [
            "https://tamil.thehindu.com/sitemap.xml",
            "https://tamil.thehindu.com/sitemap/archive.xml",
        ],
        "archive_pages": [
            "https://tamil.thehindu.com/tamilnadu/",
    ...
    },
    {
        "name": "OneIndia Tamil",
        "sitemaps": [
            "https://tamil.oneindia.com/sitemap.xml",
        ],
        "archive_pages": [
            "https://tamil.oneindia.com/news/tamil-nadu/",
            "https://tamil.oneindia.com/news/india/",
            "https://tamil.oneindia.com/news/world/",
        ],
    },
    {
        "name": "Vikatan",
        ...
        "archive_pages": [
            "https://www.vikatan.com/government-and-politics",
            "https://www.vikatan.com/news/tamilnadu",
        ],
    },
    {
        "name": "Puthiyathalaimurai",
        "sitemaps": [
            "https://www.puthiyathalaimurai.com/sitemap.xml",
        ],
        "archive_pages": [
            "https://www.puthiyathalaimurai.com/news/tamilnadu",
            "https://www.puthiyathalaimurai.com/news/india",
        ],
    },
    {
        "name": "Tamil Mirror",
        "sitemaps": [
            "https://www.tamilmirror.lk/sitemap.xml",
            "https://www.tamilmirror.lk/sitemap_index.xml",
        ],
        "archive_pages": [
            "https://www.tamilmirror.lk/latest-news",
        ],
    },
    {
        "name": "BBC Tamil",
        "sitemaps": [],
        "archive_pages": [
            "https://www.bbc.com/tamil/topics/c2ldjn9rrdzt",
            "https://www.bbc.com/tamil/topics/cnx753jenyjt",
            "https://www.bbc.com/tamil/topics/cz74k717z97t",
        ],
    },
    {
        "name": "Thinakaran",
        "sitemaps": [
            "https://www.thinakaran.lk/sitemap.xml",
        ],
        "archive_pages": [
            "https://www.thinakaran.lk/local/",
            "https://www.thinakaran.lk/foreign/",
        ],
    },
    {
        "name": "Sudar Oli",
        "sitemaps": [
            "https://www.sudaroli.com/sitemap.xml",
        ],
        "archive_pages": [
            "https://www.sudaroli.com/news/",
        ],
    },
]
...
def extract_sentences_from_html(html: str, min_chars: int) -> list[str]:
    ...
    for tag in soup.find_all([
        ...
        tag.decompose()
    paras = []
    for css in ["article p", ".article-content p", ".node__content p",
...
def fetch_sitemap_urls(session: requests.Session, sitemap_url: str, depth: int = 0) -> list[str]:
    if depth > 2:
        return []
    try:
    ...
    if "<sitemapindex" in xml or SITEMAP_INDEX_TAG.search(xml):
        ...
        for child_url in child_urls:
            ...
            if year_m and not (2012 <= 2000 + int(year_m.group(1)) <= 2021):
                continue
            collected.extend(fetch_sitemap_urls(session, child_url, depth + 1))
        return collected
...
def extract_links_from_archive_page(session: requests.Session, page_url: str) -> list[str]:
    try:
...
def collect_urls_for_site(session: requests.Session, site: dict) -> list[str]:
    ...
    if not urls:
        for arch_url in site.get("archive_pages", []):
            ...
            found = extract_links_from_archive_page(session, arch_url)
            print(f"    -> {len(found)} URLs")
            urls.extend(found)
...
def main():
    ...
    print(f"[CONFIG] Sources    : {len(TAMIL_NEWS_SITES)} Tamil news sites")

    get_tokenizer()
    session = make_session()

    print("\n[PHASE 1] Collecting article URLs from sitemaps ...")
    all_urls: list[str] = []
    for site in TAMIL_NEWS_SITES:
        print(f"\n  [SITE] {site['name']}")
    ...
    print(f"\n[PHASE 1] Total unique URLs: {len(all_urls)}")

    print(f"\n[PHASE 2] Fetching articles with {args.workers} threads ...")
    print("=" * 60)
            ...
            for future in as_completed(futures):
                ...
                for text in samples_from_sentences(sentences, min_tok, max_tok):
                    ...
                    print(f"  [SAMPLE {sample_count}/{num_samples}] "
                          f"{tok} tok | article #{total_articles}")
