...
HINDI_FUNCTION_WORDS: list[str] = [
    "और", "लेकिन", "परंतु", "या", "तथा", "किंतु", "अथवा", "मगर", "बल्कि",
    "इसलिए", "क्योंकि", "इसीलिए", "अतः", "चूंकि", "यदि", "अगर", "जबकि",
    "तो", "यानी", "यानि", "कि", "फिर", "अन्यथा", "वरना",
    "भी", "ही", "सिर्फ", "केवल", "मात्र", "अभी", "तक",
    "वैसे", "दरअसल", "बस", "साथ",
    "बाद", "पहले", "बीच", "पास", "ऊपर", "नीचे", "अंदर", "बाहर",
    "सामने", "पीछे", "बारे", "द्वारा", "जरिए", "अलावा", "बजाय",
    "इसके", "उसके", "जैसे", "जैसा", "हालांकि",
]
...
_COMBINED_PATTERN: re.Pattern = re.compile(
    _SEP_BEFORE
    + "("
    + "|".join(re.escape(w) for w in HINDI_FUNCTION_WORDS)
    + ")"
    + _SEP_AFTER
)

_DEVANAGARI_PATTERN: re.Pattern = re.compile(r'[\u0900-\u097F]+')
...
def delete_function_words(text: str, delete_rate: float, rng: random.Random) -> str:
    ...
    if spans:
        ...
        devanagari_words = [m for m in _DEVANAGARI_PATTERN.finditer(text)]
        if not devanagari_words:
            return text

        n_delete = max(1, int(round(len(devanagari_words) * delete_rate)))
        n_delete = min(n_delete, len(devanagari_words))
        chosen_words: set[int] = set(rng.sample(range(len(devanagari_words)), n_delete))

        parts: list[str] = []
        prev_end: int = 0

        for idx, m in enumerate(devanagari_words):
...
if __name__ == "__main__":
    ...
    json_file = r"/Users/user/Desktop/IndicDetect/Benchmark_Data/Task_Data/Adverisal_Attacks/Hindi/Petrubation_Attacks/Test.json"
