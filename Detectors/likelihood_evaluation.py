...
def experiment(args):
    ...
    for item in tqdm.tqdm(threshold_data, desc="Processing threshold data"):
        score = get_ll(item["text"], args, base_tokenizer, base_model)
    ...
    for filename in filenames:
        ...
        for item in tqdm.tqdm(test_data, desc=os.path.basename(filename)):
            score = get_ll(item["text"], args, base_tokenizer, base_model)
            item["text_ll"] = score if np.isfinite(score) else None
        ...
        data_file = os.path.join(output_dir, f"{unique_name}_likelihood_data.json")
        result_file = os.path.join(output_dir, f"{unique_name}_likelihood_result.json")
